#!/usr/bin/env python3
# build-local-quality.sh — generates self-contained local-quality.sh
# Run: python3 scripts/quality/build-local-quality.sh
# Or:  bash scripts/quality/build-local-quality.sh  (shebang routes to python3)
...
OUTPUT = os.path.join(QUALITY_DIR, "local-quality.sh")
...
SCRIPT = r"""#!/bin/bash
# local-quality.sh — self-contained Flutter quality check
# Usage: bash scripts/quality/local-quality.sh [--dev|--sit]
# Requires: Java 17, Flutter, curl, unzip, python3
#
# Copy this file + flutter-lint.sh into scripts/quality/ and run:
...
show_help() {
  ...
  echo "  -d, --dup-threshold  Max duplication % (default: 1)"
  echo "  -c, --cov-threshold  Min coverage % (default: 95)"
...
diagnose_sonar_failure() {
  ...
  if [[ -z "$java_ver" ]]; then
    ...
    echo -e "         then re-run: bash scripts/quality/local-quality.sh"
    return
  ...
  echo -e "  → Try: bash scripts/quality/local-quality.sh"
}
...
LINT_MODE_DISPLAY="${LINT_MODE:-strict}"
echo ""
echo -e "${CYAN}${PROJECT_NAME}  |  ${LINT_MODE_DISPLAY}  |  ${RUN_DATE}${NC}"
echo -e "${CYAN}────────────────────────────────────────────────────${NC}"
...
if [[ -f "$LINT_SCRIPT" ]]; then
  _spin_start "Lint"
  if [[ -n "$LINT_MODE" ]]; then
    LINT_LINE="$(bash "$LINT_SCRIPT" "--$LINT_MODE" 2>&1 | tail -1 || true)"
  else
  ...
  echo -e "${YELLOW}▸ Lint   — skipped (flutter-lint.sh not found)${NC}"
fi
...
cov = d.get('coverage',{})
dup = d.get('duplicate',{})
cov_pct = cov.get('percentage'); cov_thr = cov.get('threshold', 90)
dup_pct = dup.get('percentage'); dup_thr = dup.get('threshold', 1)
