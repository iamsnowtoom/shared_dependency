...
if [[ "$MODE" == "dev" || "$MODE" == "sit" ]]; then
  if [[ "$ERROR_COUNT" -gt 0 ]]; then
    ...
    echo -e "  ${RED}✗ Lint  — $ERROR_COUNT errors, $WARNING_COUNT warnings, $INFO_COUNT info  [${ELAPSED}s]${NC}"
  else
    write_lint_json "true"
    if [[ "$WARNING_COUNT" -gt 0 || "$INFO_COUNT" -gt 0 ]]; then
      echo -e "  ${YELLOW}⚠ Lint  — no errors  ($WARNING_COUNT warnings, $INFO_COUNT info)  [${ELAPSED}s]${NC}"
    else
      echo -e "  ${GREEN}✓ Lint  — clean  [${ELAPSED}s]${NC}"
    fi
  fi
  exit 0
fi
